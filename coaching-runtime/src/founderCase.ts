/**
 * The founder's actual real retry text, used throughout tonight's session
 * (the notebookBullets() fix, the Interviewer's Notebook contradiction
 * fix). This is NOT byte-identical to evidence-runtime's Case 002 fixture
 * (which reproduces the Atlas's own Case 002 wording) — a real, small
 * difference exists: the Atlas says "a way of locking IT in," the founder's
 * actual retry says "a way of locking THE PART in." The em dash at the end
 * also differs (Atlas fixture uses a plain hyphen). Flagged here rather
 * than silently treated as identical — see README.md.
 *
 * Fixture-mode Listen Engine proposals below are adapted from
 * evidence-runtime/fixtures/case002.ts's own (already independent) reading,
 * changed only where the transcript itself differs.
 */
...
export const LISTEN_ENGINE_FIXTURE: ListenEngineRawOutput = {
  class_a_proposals: [
    ...
    { proposal_id: "a_need", claim_type: "action", quote: "I needed to come up with a way of locking the part in and prevent it from moving" },
